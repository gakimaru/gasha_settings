...
#define GASHA_STDIN_IS_AVAILABLE//標準入力を使用した処理を有効化する場合、このマクロを有効化する
                                //※標準入力でバッグポーズで、ポーズ状態の解除待ちに標準入力を利用。
...
//#define GASHA_STR_POOL_ALLWAYS_TOGETHER_CPP_H//.hファイルのインクルードに伴い、常に.cpp.hファイル（および.inlファイル）を自動インクルードする場合は、このマクロを有効にする
...
#define GASHA_PROFILER_SECURE_INITIALIZE//プロファイラ使用時に確実にコンテナを初期化したい場合は、このマクロを有効にする
                                        //※プロファイラの明示的な初期化が不要になる。
                                        //※パフォーマンスに問題がある場合はこのマクロを無効にすること。
                                        //　その場合、最初のプロファイラ使用前に、明示的な初期化が必要。
...
//#define GASHA_PROFILER_WITHOUT_THREAD_SAFE//プロファイラをスレッドセーフにしない場合は、このマクロを有効にする
                                            //※デフォルトはスレッドセーフ。
