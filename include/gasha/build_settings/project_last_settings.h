...
#define GASHA_LOG_MASK_SECURE_INITIALIZE//ログマスク使用時に確実にグローバルログマスクを初期化したい場合は、このマクロを有効にする
                                        //※ログマスクの明示的な初期化が不要になる。
                                        //※パフォーマンスに問題がある場合はこのマクロを無効にすること。
                                        //　その場合、最初のログマスク使用前に、明示的な初期化が必要。

//#define GASHA_DEFAULT_LOG_MASK_OF_LOG 1//デフォルトのログマスク
//#define GASHA_DEFAULT_LOG_MASK_OF_NOTICE 4//デフォルトの画面通知ログマスク
//※それぞれ変更したい場合、このマクロを任意に設定する。
//※ログマスク：
//  0 ... asDetail：   詳細／冗長メッセージ
//  1 ... asNormal：   通常メッセージ
//  2 ... asImportant：重要メッセージ
//  3 ... asWarning：  警告メッセージ
//  4 ... asCritical： 重大メッセージ
//  5 ... asAbsolute： 絶対メッセージ
//  6 ... asSilentAbsolutely：絶対静寂（表示しない）
